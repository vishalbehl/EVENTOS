...
export function registerMediaBlocks(editor: Editor) {
  ...
  bm.add('media-image', {
    ...
    category: 'Media',
    content: `<img data-gjs-type="image" src="https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&auto=format&fit=crop" alt="Event image" loading="lazy" style="width: 100%; height: auto; border-radius: 16px; object-fit: cover; display: block;" />`,
  });
  ...
  bm.add('media-video-youtube', {
    ...
    category: 'Media',
    content: `<div data-gjs-type="image" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; border-radius: 16px; background: var(--surface);">
  ...
  bm.add('media-gallery-grid', {
    ...
    category: 'Media',
    content: `
      <div data-gjs-type="image" style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; padding: 20px; box-sizing: border-box;">
        ${[
  ...
  bm.add('media-image-text-left', {
    ...
    category: 'Media',
    content: `
      <div data-gjs-type="image" style="display: flex; gap: 48px; align-items: center; flex-wrap: wrap; padding: 40px 24px; box-sizing: border-box;">
        ...
        <div style="flex: 1; min-width: 280px;">
          ...
          <h2 style="font-size: 36px; font-weight: 800; color: var(--foreground); line-height: 1.2; margin: 0 0 16px 0;">Where Ideas Shape the Future</h2>
          <p style="font-size: 16px; color: var(--muted-foreground); line-height: 1.75; margin: 0 0 24px 0;">Join thousands of experts, innovators, and thought leaders for three transformative days of discovery, debate, and connection.</p>
          <a href="#" style="display: inline-flex; align-items: center; background: var(--pri, var(--primary)); color: var(--background); padding: 13px 28px; border-radius: 10px; font-weight: 700; text-decoration: none; font-size: 14px;">Learn More</a>
        </div>
      </div>`,
  });
  ...
  bm.add('media-logo-marquee', {
    ...
    category: 'Media',
    content: `
      <div data-gjs-type="image" style="padding: 24px; overflow: hidden; box-sizing: border-box;">
        <p style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted-foreground); text-align: center; margin: 0 0 20px 0;">Supported By</p>
        <div style="display: flex; justify-content: center; align-items: center; gap: 48px; flex-wrap: wrap; opacity: 0.7;">
          ${['MICROSOFT', 'GOOGLE', 'AWS', 'CISCO', 'IBM', 'ORACLE'].map(n => `<div style="font-size: 18px; font-weight: 900; letter-spacing: -0.02em; color: var(--foreground);">${n}</div>`).join('')}
        </div>
      </div>`,
  });
  ...
  bm.add('media-icon-block', {
    ...
    category: 'Media',
    content: `
      <div data-gjs-type="image" style="display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 24px; text-align: center;">
        ...
        <h4 style="font-size: 16px; font-weight: 700; color: var(--foreground); margin: 0;">Feature Title</h4>
        <p style="font-size: 14px; color: var(--muted-foreground); line-height: 1.6; margin: 0;">Brief description of this feature or benefit.</p>
      </div>`,
  });
}
