...
export function registerOverviewBlocks(editor: Editor, s?: EventDataSnapshot) {
      ...
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box;">
        <div style="max-width: 1100px; margin: 0 auto; display: flex; gap: 64px; align-items: center; flex-wrap: wrap;">
          <div style="flex: 1; min-width: 280px;">
            <img src="${banner}" alt="About event" loading="lazy" style="width: 100%; height: 400px; object-fit: cover; border-radius: 24px; display: block;" />
          </div>
          <div style="flex: 1; min-width: 280px;">
            <p style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.15em; color: var(--pri, var(--primary)); margin: 0 0 12px 0;">About the Event</p>
            <h2 style="font-size: 38px; font-weight: 900; color: var(--foreground); line-height: 1.15; margin: 0 0 20px 0; letter-spacing: -0.02em;">A Conference That Changes Everything</h2>
            <p style="font-size: 16px; color: var(--muted-foreground); line-height: 1.75; margin: 0 0 28px 0;">${desc}</p>
  ...
  bm.add('overview-centered', {
    ...
    category: 'Event Overview',
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box; text-align: center;">
        <div style="max-width: 760px; margin: 0 auto 56px auto;">
          <p style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.15em; color: var(--pri, var(--primary)); margin: 0 0 12px 0;">About the Conference</p>
          <h2 style="font-size: 42px; font-weight: 900; color: var(--foreground); line-height: 1.15; margin: 0 0 20px 0; letter-spacing: -0.02em;">Where Innovation Meets Academia</h2>
          <p style="font-size: 17px; color: var(--muted-foreground); line-height: 1.75; margin: 0;">${desc}</p>
        </div>
  ...
  bm.add('organizer-message', {
    ...
    category: 'Event Overview',
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--muted); border-top: 1px solid color-mix(in srgb, var(--primary) 10%, transparent); border-bottom: 1px solid color-mix(in srgb, var(--primary) 10%, transparent); box-sizing: border-box;">
        <div style="max-width: 860px; margin: 0 auto; display: flex; gap: 48px; align-items: flex-start; flex-wrap: wrap;">
          <div style="flex-shrink: 0; display: flex; flex-direction: column; align-items: center; gap: 12px; width: 160px;">
            ...
            <div style="text-align: center;">
              <div style="font-size: 15px; font-weight: 800; color: var(--foreground);">${organizer.name}</div>
              <div style="font-size: 13px; color: var(--muted-foreground); margin-top: 2px;">${organizer.designation || 'Conference Chair'}</div>
            </div>
          </div>
          <div style="flex: 1; min-width: 240px;">
            ...
            <blockquote style="font-size: 17px; font-style: italic; color: var(--foreground); line-height: 1.75; margin: 0 0 20px 0; border: none; padding: 0;">${organizer.message || 'Welcome to this landmark event. We have curated an outstanding program and I look forward to welcoming you.'}</blockquote>
...
export function registerStatisticsBlocks(editor: Editor, s?: EventDataSnapshot) {
  ...
  bm.add('stats-4col', {
    ...
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box;">
        <div style="max-width: 1000px; margin: 0 auto; display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 24px;">
          ${stats.map(st => `
            <div style="text-align: center; padding: 32px 20px; background: var(--muted); border: 1px solid var(--muted); border-radius: 20px;">
  ...
  bm.add('stats-banner', {
    ...
    category: 'Event Statistics',
    content: `
      <section data-gjs-type="section" style="padding: 48px 24px; background: linear-gradient(135deg, var(--pri, var(--primary)) 0%, var(--primary) 100%); box-sizing: border-box;">
        <div style="max-width: 1000px; margin: 0 auto; display: flex; justify-content: center; align-items: center; flex-wrap: wrap; gap: 0;">
          ${stats.map((st, i) => `
            <div style="flex: 1; min-width: 160px; text-align: center; padding: 20px; ${i > 0 ? 'border-left: 1px solid var(--muted);' : ''}">
...
export function registerCountdownBlocks(editor: Editor, s?: EventDataSnapshot) {
  ...
  bm.add('countdown-minimal', {
    ...
    content: `
      <section data-gjs-type="section" style="padding: 60px 24px; background: var(--base, var(--background)); text-align: center; box-sizing: border-box;" data-countdown="${startDate}">
        <p style="font-size: 14px; font-weight: 700; color: var(--muted-foreground); margin: 0 0 24px 0; text-transform: uppercase; letter-spacing: 0.1em;">Conference Starts In</p>
        <div style="display: flex; gap: 12px; justify-content: center; align-items: center; flex-wrap: wrap;">
          ${[['45', 'Days'], ['12', 'Hours'], ['38', 'Min'], ['52', 'Sec']].map(([n, l], i) => `
            <div style="text-align: center;">
              <div style="font-size: 56px; font-weight: 900; color: var(--foreground); line-height: 1; font-variant-numeric: tabular-nums;">${n}</div>
              <div style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted-foreground); margin-top: 4px;">${l}</div>
            </div>
  ...
  bm.add('countdown-glass', {
    ...
    category: 'Countdown Timers',
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: linear-gradient(135deg, var(--background) 0%, var(--background) 100%); text-align: center; box-sizing: border-box;">
        ...
        <h2 style="font-size: 32px; font-weight: 900; color: var(--foreground); margin: 0 0 40px 0; letter-spacing: -0.02em;">Event Starts In</h2>
        <div style="display: flex; gap: 20px; justify-content: center; flex-wrap: wrap; max-width: 600px; margin: 0 auto 40px auto;">
          ${[['45', 'Days'], ['12', 'Hours'], ['38', 'Minutes'], ['52', 'Seconds']].map(([n, l]) => `
            <div style="flex: 1; min-width: 100px; background: var(--bg-surface-hover, var(--muted)); backdrop-filter: blur(16px); border: 1px solid var(--border-default, var(--muted)); border-radius: 20px; padding: 24px 16px; text-align: center; box-shadow: 0 8px 24px var(--muted);">
              ...
              <div style="font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted-foreground); margin-top: 8px;">${l}</div>
            </div>`).join('')}
        </div>
        <a href="#register" style="display: inline-block; background: var(--pri, var(--primary)); color: var(--background); padding: 14px 36px; border-radius: 10px; font-weight: 800; font-size: 15px; text-decoration: none; box-shadow: 0 8px 20px color-mix(in srgb, var(--primary) 35%, transparent);">Register Now</a>
      </section>`,
  });
  ...
  bm.add('countdown-banner', {
    ...
    category: 'Countdown Timers',
    content: `
      <div data-gjs-type="section" style="background: linear-gradient(135deg, var(--pri, var(--primary)) 0%, #7c3aed 100%); padding: 16px 24px; display: flex; align-items: center; justify-content: center; gap: 24px; flex-wrap: wrap; box-sizing: border-box;">
    ...
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box;">
        <div style="max-width: 1100px; margin: 0 auto; display: flex; gap: 48px; align-items: flex-start; flex-wrap: wrap;">
          <div style="flex: 1; min-width: 280px;">
            <p style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.15em; color: var(--pri, var(--primary)); margin: 0 0 12px 0;">Conference Venue</p>
            <h2 style="font-size: 36px; font-weight: 900; color: var(--foreground); margin: 0 0 12px 0; letter-spacing: -0.02em;">${venue.name}</h2>
            <p style="font-size: 15px; color: var(--muted-foreground); margin: 0 0 20px 0; line-height: 1.7;">${venue.description || ''}</p>
    ...
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box;">
        <div style="max-width: 1100px; margin: 0 auto;">
          <div style="text-align: center; margin-bottom: 48px;">
            <h2 style="font-size: 38px; font-weight: 900; color: var(--foreground); margin: 0 0 8px 0; letter-spacing: -0.02em;">Gallery</h2>
            <p style="font-size: 15px; color: var(--muted-foreground);">Highlights from past editions</p>
          </div>
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px;">
            ${photos.map((url, i) => `<div style="aspect-ratio: ${i === 0 || i === 5 ? '1/1' : '1/1'}; overflow: hidden; border-radius: 14px; cursor: pointer;"><img src="${url}&auto=format&fit=crop" alt="Gallery photo ${i + 1}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.3s;" /></div>`).join('')}
          </div>
        </div>
      </section>`,
  });
...
export function registerContactFooterBlocks(editor: Editor, s?: EventDataSnapshot) {
    ...
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; background: var(--base, var(--background)); box-sizing: border-box;">
        <div style="max-width: 1000px; margin: 0 auto; display: flex; gap: 64px; flex-wrap: wrap;">
          <div style="flex: 1; min-width: 240px;">
            <p style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.15em; color: var(--pri, var(--primary)); margin: 0 0 12px 0;">Get In Touch</p>
            <h2 style="font-size: 34px; font-weight: 900; color: var(--foreground); margin: 0 0 20px 0; letter-spacing: -0.02em;">Contact Us</h2>
          ...
          <div style="flex: 1; min-width: 280px; background: var(--muted); border: 1px solid var(--border); border-radius: 20px; padding: 28px; box-sizing: border-box;">
            ...
            <form style="display: flex; flex-direction: column; gap: 14px;">
              ...
              <button type="submit" style="background: var(--pri, var(--primary)); color: var(--background); border: none; padding: 12px 24px; border-radius: 8px; font-weight: 700; font-size: 14px; cursor: pointer; align-self: flex-start;">Send Message</button>
            </form>
          </div>
        </div>
      </section>`,
  ...
  bm.add('footer-multi-column', {
    ...
    category: 'Contact & Footer',
    content: `
      <footer data-gjs-type="section" style="background: var(--surface); border-top: 1px solid var(--border-subtle, var(--muted)); padding: 60px 24px 32px; box-sizing: border-box;">
        <div style="max-width: 1100px; margin: 0 auto;">
          <div style="display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 40px; margin-bottom: 48px; flex-wrap: wrap;">
            <div>
              <div style="font-size: 22px; font-weight: 900; color: var(--background); margin-bottom: 12px; letter-spacing: -0.02em;">${eventName}</div>
              <p style="font-size: 14px; color: var(--muted-foreground); line-height: 1.7; margin: 0 0 20px 0; max-width: 280px;">The premier international conference bringing together the world's leading researchers and practitioners.</p>
            ...
            ${[
              ...
              <div>
                <h4 style="font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: 0.12em; color: var(--muted-foreground); margin: 0 0 16px 0;">${col.title}</h4>
  ...
  bm.add('footer-simple', {
    ...
    category: 'Contact & Footer',
    content: `
      <footer data-gjs-type="section" style="background: var(--surface); border-top: 1px solid var(--border-subtle, var(--muted)); padding: 28px 24px; box-sizing: border-box; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 12px;">
  ...
  bm.add('marketing-cta-register', {
    ...
    content: `
      <section data-gjs-type="section" style="padding: 80px 24px; text-align: center; background: linear-gradient(135deg, var(--background) 0%, var(--muted) 50%, var(--background) 100%); box-sizing: border-box;">
