...
Latest identity/organization evidence (2026-07-15): Command Center role and permission administration uses explicit organization-scoped `/superadmin/access` contracts with support-purpose headers, transaction-local tenant context, step-up-protected mutations, audit evidence, stale-version rejection, assignment-safe archival, and cross-tenant concealment. Tenant provisioning is now reasoned, step-up protected, idempotent, audited, and returns a one-time owner invitation handoff. Organization membership supports invitation, role lifecycle, session-revoking removal, and tenant-scoped event workspace assignment guarded by plan seat limits and database uniqueness. A SQLAlchemy tenant-filter cache defect that reused the first organization UUID was replaced with explicit bound criteria per tenant-owned model and a sequential-tenant regression. Nine backend tests, one focused tenant-switch test, two frontend organization-admin tests, 65 frontend tests, six Playwright/axe journeys, Python compilation, TypeScript, OpenAPI drift, zero-error lint, production build, and Alembic upgrade to `event_assignment_unique_0710` pass.
...
## Support, Access Review, and Activation Control Batch Result

- `/support-center/tickets` and its detail route now use explicit tenant support scope and real cursor, detail, comment, assignment, escalation, SLA, and versioned lifecycle contracts; the two routes were removed from the production mock allowlist.
- Customer replies and internal notes persist through separate policy-aware behavior, and internal notes remain absent from tenant-facing comment responses.
- Lifecycle mutations require recent step-up authentication, a current optimistic version, an operator-entered reason, transaction-local tenant context, and immutable sensitive audit evidence.
- `/identity-security/access-reviews` now persists periodic, role-change, and break-glass reviews. Break-glass self-approval is rejected, and approval never directly assigns a platform role or RLS bypass.
- Activation inspection now exposes idempotent transfer and deactivation controls through the existing activation service, preserving meaningful-usage policy, consumption lineage, snapshot-first enforcement, and audit behavior.
- Verification passes: 32 focused backend tests, 67 frontend tests, generated OpenAPI drift, frontend contract and zero-debt production-mock gates, zero-error ESLint, 86-page optimized build, nine Playwright/axe journeys, Python compilation, and Alembic head `support_attachment_safety_0760`.
- Support attachments now use tenant-scoped presigned upload, authoritative object metadata verification, quarantine, shared malware-scan handoff, status polling, `READY`-only download, idempotency, audit, forced RLS and isolation tests.
- CRM activities, tasks and notes now use real tenant-scoped lifecycle APIs and UI with entity/assignee validation, optimistic versions, idempotency, reasons, audit, archive/restore, cursor pagination and forced RLS.
- The knowledge-base and notification surfaces no longer fabricate operational records; the production mock allowlist is empty and browser regression evidence passes. Remaining Phase 3 closure is live broker/object-storage/scanner evidence, deployment cache evidence, and independent manual accessibility review.
