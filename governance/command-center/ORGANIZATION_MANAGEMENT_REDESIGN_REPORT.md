...
- Added a development-only, idempotent Northstar Events demonstration seed.
...
From `services/backend`, after applying migrations and initializing catalog data:

```powershell
.venv/Scripts/python.exe ops/seed_organization_dossier_demo.py
```

The command refuses to run in production and prints the generated organization route. It is safe to run repeatedly.
...
- Demo seed executed successfully against the migrated development database; the generated Northstar dossier contains a plan, 12 event units, three event records, 53 catalog capabilities, one assigned premium add-on, and one paid invoice.
