...
async def _self_checkin_payload(db: AsyncSession, participant: Participant) -> Dict[str, Any]:
    ...
    allowed_self_statuses = getattr(policy, "allowed_payment_statuses_for_self_checkin", None) or ["All"]
    payment_locked = not is_payment_status_allowed(p_status, allowed_self_statuses)
...
async def get_registration_summary(db: AsyncSession = Depends(get_database)):
    ...
    payment_status_rows = (await db.execute(
        select(
            func.coalesce(func.nullif(Participant.paid_status, ''), 'Unspecified').label("status"),
