...
class AgendaCatalogQueryService:
    ...
    async def list_session_types(self) -> list[SessionType]:
        statement = (
            select(SessionType)
            .options(load_only(
                SessionType.id, SessionType.organization_id, SessionType.name,
            ...
            .order_by(SessionType.name, SessionType.id)
            .limit(100)
        )
        return list((await self.db.scalars(statement)).all())
    ...
    async def list_room_types(self) -> list[RoomType]:
        statement = (
            ...
            .order_by(RoomType.name, RoomType.id)
            .limit(100)
        )
        return list((await self.db.scalars(statement)).all())

    async def list_track_types(self) -> list[TrackType]:
        statement = (
            select(TrackType)
            .options(load_only(
                TrackType.id, TrackType.organization_id, TrackType.name,
            ...
            .where(TrackType.is_active.is_(True))
            .order_by(TrackType.name, TrackType.id)
            .limit(100)
        )
        return list((await self.db.scalars(statement)).all())
