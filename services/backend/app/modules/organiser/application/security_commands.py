...
class OrganizerSecurityCommandService:
    ...
    async def update_policy(self, *, organization_id, actor, values: dict, if_match: int, idempotency_key: str | None = None) -> dict:
        try:
            idem = None
            if idempotency_key:
                idem = await begin_idempotent(
                    self.db, organization_id=organization_id, actor_id=actor.id,
                    operation="organiser.organization.security.update", key=idempotency_key,
                ...
                if replay_response(idem) is not None:
                    ...
                    if row is None:
                        raise RuntimeError("Completed security-policy idempotency resource is missing.")
            ...
            if idem is not None:
                await complete_idempotent(
                    self.db, idem, response_status=200, response_body=state, resource_id=row.id
                )
            await self.db.commit()
    ...
    @staticmethod
    def _state(row: OrganizationSecurityPolicy) -> dict:
        return {
