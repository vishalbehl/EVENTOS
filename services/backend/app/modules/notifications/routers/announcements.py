...
async def create_announcement(
    ...
    return await AnnouncementCommandService.create(
        db,
        event=event,
...
async def delete_announcement(
    ...
    return await AnnouncementCommandService.archive(
        db,
        event_id=event.id,
...
async def get_presigned_download_url(
    ...
    try:
        if settings.STORAGE_MODE == "local":
            ...
            url = await asyncio.to_thread(upload_service.create_presigned_download,
                bucket=bucket,
                storage_path=storage_path,
                expiry_seconds=1200, # 20 minutes short-lived URL
                verified_organization_id=matched[1].organization_id,
            )
        return {"url": url}
    except Exception as e:
        logger.exception("announcement_signed_url_failed bucket={} storage_class={}", bucket, type(e).__name__)
