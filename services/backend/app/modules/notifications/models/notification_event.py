...
class NotificationEvent(Base):
    ...
    Tracks the lifecycle of a notification from queuing to delivery or failure.
    """
    ...
    provider_id: Mapped[Optional[str]] = mapped_column(String(255), nullable=True) # SendGrid Msg ID, etc.
    provider_response: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)
    latency_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True) # Time from queue to sent
