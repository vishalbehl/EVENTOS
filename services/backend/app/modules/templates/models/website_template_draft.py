...
class WebsiteTemplate(Base):
    ...
    category_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("operation_templates.template_categories.id", ondelete="CASCADE"))
...
class WebsiteTemplateDraft(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    template_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("operation_templates.templates.id", ondelete="CASCADE"), nullable=False)
