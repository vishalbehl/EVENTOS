# app/modules/platform/departments/models.py
import uuid
...
class Department(Base, SoftDeleteMixin):
    ...
    roles: Mapped[List["DepartmentRole"]] = relationship(
        "DepartmentRole",
        back_populates="department",
        cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        Index("idx_dept_org_code", "organization_id", "code"),
...
class DepartmentMember(Base, SoftDeleteMixin):
    ...
    def __repr__(self) -> str:
        return f"<DepartmentMember id={self.id} dept_id={self.department_id} user_id={self.user_id}>"
