...
class OrganizationLocation(Base):
    ...
    team_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("organizer_access.organization_teams.id", ondelete="SET NULL"), nullable=True)
...
class OrganizationTeam(Base):
    ...
    owner_member_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("organizer_access.organization_members.id", ondelete="SET NULL"), nullable=True)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="ACTIVE")
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now, onupdate=_now)
    deleted_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    __table_args__ = (Index("uq_organization_team_name_active", "organization_id", "name", unique=True, postgresql_where=text("deleted_at IS NULL")), Index("ix_organization_teams_org_active", "organization_id", "deleted_at"), {"schema": "organizer_access"})
...
class OrganizationTeamMember(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("platform.organizations.id", ondelete="CASCADE"), nullable=False)
    team_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("organizer_access.organization_teams.id", ondelete="CASCADE"), nullable=False)
    organization_member_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("organizer_access.organization_members.id", ondelete="CASCADE"), nullable=False)
    created_by: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="RESTRICT"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now)
    __table_args__ = (UniqueConstraint("team_id", "organization_member_id", name="uq_organization_team_member"), Index("ix_organization_team_members_org_team", "organization_id", "team_id"), {"schema": "organizer_access"})
...
class OrganizationTeamEvent(Base):
    ...
    created_by: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="RESTRICT"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now)
    __table_args__ = (UniqueConstraint("team_id", "event_id", name="uq_organization_team_event"), Index("ix_organization_team_events_org_team", "organization_id", "team_id"), {"schema": "organizer_access"})


class OrganizationCustomField(Base):
    ...
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now, onupdate=_now)
    __table_args__ = (
        UniqueConstraint("organization_id", "field_key", name="uq_organization_custom_fields_org_key"),
        UniqueConstraint("organization_id", "idempotency_key", name="uq_organization_custom_fields_org_idempotency"),
        Index("ix_organization_custom_fields_org_active", "organization_id", "is_active"),
...
class OrganizationDocument(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_group_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), nullable=False, default=uuid.uuid4)
    revision: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    is_current: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    organization_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("platform.organizations.id", ondelete="CASCADE"), nullable=False)
    asset_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("content.assets.id", ondelete="RESTRICT"), nullable=False)
    ...
    __table_args__ = (
        Index("ix_organization_documents_org_active", "organization_id", "archived_at"),
        UniqueConstraint("organization_id", "document_group_id", "revision", name="uq_organization_document_revision"),
        Index("ix_organization_documents_group_current", "organization_id", "document_group_id", "is_current"),
        {"schema": "platform"},
    )


class OrganizationApprovalRule(Base):
    __tablename__ = "organization_approval_rules"
    ...
    approver_chain: Mapped[list[dict[str, Any]]] = mapped_column(JSONB, nullable=False, default=list)
    conditions: Mapped[dict[str, Any]] = mapped_column(JSONB, nullable=False, default=dict)
    status: Mapped[str] = mapped_column(String(24), nullable=False, default="ACTIVE")
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    created_by: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="RESTRICT"), nullable=False)
    ...
    __table_args__ = (
        Index("uq_org_approval_rule_name_active", "organization_id", "name", unique=True, postgresql_where=text("archived_at IS NULL")),
        Index("ix_org_approval_rules_org_domain", "organization_id", "domain", "status"),
        {"schema": "organizer_access"},
    )


class OrganizationAttentionState(Base):
    __tablename__ = "organization_attention_states"
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "task_key", name="uq_organization_attention_task"),
        Index("ix_organization_attention_org_status", "organization_id", "status", "snoozed_until"),
        {"schema": "organizer_access"},
    )
...
class EventCommercialContract(Base):
    ...
    created_by: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="RESTRICT"), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now)
    __table_args__ = (UniqueConstraint("event_id", "version", name="uq_event_contract_version"), Index("ix_event_contract_active", "organization_id", "event_id", "status"), {"schema": "commerce"})
...
class EntitlementOverrideRequest(Base):
    ...
    __table_args__ = (UniqueConstraint("organization_id", "idempotency_key", name="uq_entitlement_override_idempotency"), Index("ix_entitlement_override_scope_status", "organization_id", "event_id", "status"), {"schema": "commerce"})
...
class EntitlementShadowComparison(Base):
    ...
    __table_args__ = (Index("ix_entitlement_shadow_scope_time", "organization_id", "event_id", "compared_at"), {"schema": "commerce"})
...
class PrivilegedAccessSession(Base):
    ...
    revoked_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now)
    __table_args__ = (Index("ix_privileged_access_actor_active", "organization_id", "actor_user_id", "expires_at"), {"schema": "command_center_audit"})
...
class OrganizationFinancialAdjustment(Base):
    ...
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    __table_args__ = (UniqueConstraint("organization_id", "idempotency_key", name="uq_org_financial_adjustment_idempotency"), Index("ix_org_financial_adjustment_status", "organization_id", "status", "created_at"), {"schema": "commerce"})
...
class CommercialAccessRequest(Base):
    ...
    requested_plan_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("commerce.subscription_plans.id", ondelete="RESTRICT"), nullable=False)
    ...
    decided_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    applied_subscription_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("commerce.organization_subscriptions.id", ondelete="SET NULL"), nullable=True)
    idempotency_key: Mapped[str] = mapped_column(String(160), nullable=False)
    version: Mapped[int] = mapped_column(Integer, nullable=False, default=1)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_now, onupdate=_now)
