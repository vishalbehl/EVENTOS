...
class VenueSyncJob(Base):
    ...
    bytes_transferred: Mapped[Optional[int]] = mapped_column(Integer)
    transfer_speed_mbps: Mapped[Optional[float]] = mapped_column(Float)
    checksum_verified: Mapped[bool] = mapped_column(Boolean, default=False)
    integrity_hash: Mapped[Optional[str]] = mapped_column(String(64)) # SHA256
    
    worker_metadata: Mapped[Optional[dict]] = mapped_column(JSONB) # {node_id, version, retry_history}
