...
from sqlalchemy import String, Text, DateTime, Index, ForeignKey, Integer
...
class DataExport(Base):
    ...
    export_type: Mapped[str] = mapped_column(String(80), nullable=False, default="event_summary")
    file_format: Mapped[str] = mapped_column(String(12), nullable=False, default="xlsx")
...
class AccessReview(Base):
    ...
    target_user_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    reviewer_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("identity.users.id", ondelete="SET NULL"), nullable=True)
    review_type: Mapped[str] = mapped_column(String(30), nullable=False, default="PERIODIC", server_default="PERIODIC")
