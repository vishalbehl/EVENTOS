...
async def create_audit_export(
    ...
    async with TenantContextGuard.scoped(db, support_scope.organization_id):
        ...
        export = DataExport(
            ...
            request_metadata={**request_payload, "reason": payload.reason},
        )
        db.add(export)
        await db.flush()
        db.add(AuditLog(
            ...
            change_diff={"reason": payload.reason, "filters": request_payload},
            is_sensitive=True,
        ))
        await db.commit()
        try:
...
async def list_audit_exports(
    ...
    async with TenantContextGuard.scoped(db, support_scope.organization_id):
        exports = (await db.scalars(select(DataExport).where(
            DataExport.organization_id == support_scope.organization_id,
            DataExport.source_type == "audit_log_export",
