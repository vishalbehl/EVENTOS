...
async def seed_form_builder():
    print("Starting Form Builder DB setup & seeding...")
    async with AsyncSessionLocal() as session:
        # 1. Create table registration.form_categories
        ...
        """))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_categories_org_id ON registration.form_categories(organization_id)"))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_categories_slug ON registration.form_categories(slug)"))

        # 2. Create table registration.form_templates
        await session.execute(text("""
        ...
        """))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_templates_cat_id ON registration.form_templates(category_id)"))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_templates_org_id ON registration.form_templates(organization_id)"))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_templates_cat_key ON registration.form_templates(category_key)"))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_templates_scope ON registration.form_templates(scope_type)"))
        await session.execute(text("CREATE INDEX IF NOT EXISTS ix_form_templates_slug ON registration.form_templates(slug)"))
        ...
        print("Form Builder DB tables and default templates seeded successfully!")


if __name__ == "__main__":
    asyncio.run(seed_form_builder())
