...
@pytest.mark.skipif(
    ...
    reason="Requires Alembic policies and a provisioned non-owner Event_runtime role.",
)
...
async def test_runtime_role_passes_application_startup_gate() -> None:
    ...
    try:
        async with engine.begin() as connection:
            await connection.execute(text('SET LOCAL ROLE "Event_runtime"'))
            session = AsyncSession(bind=connection, expire_on_commit=False)
            try:
                report = await enforce_runtime_database_security(session)
                assert report.role_name == "Event_runtime"
