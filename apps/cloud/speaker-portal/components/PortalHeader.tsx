...
import { 
  ...
  ChevronRight, Settings, HelpCircle, Bell
} from "lucide-react";
import { cn } from "@/lib/utils";

export function PortalHeader({ speakerName, token }: { speakerName: string, token: string }) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full h-[100px] flex items-center px-6 md:px-10 gap-10 glass-3d border-t-0 border-x-0 rounded-none bg-[var(--base)]/40 backdrop-blur-xl">
      ...
      <div className="flex items-center gap-4 flex-1">
        <Link href={`/${token}`} className="flex items-center gap-4 group">
      ...
      <div className="flex items-center gap-4 md:gap-6 justify-end">
        ...
        <div className="flex items-center gap-4 pl-6 border-l border-white/10 relative" ref={userMenuRef}>
          <div className="hidden md:flex flex-col items-end mr-2">
            ...
            <p className="text-[9px] font-black text-muted uppercase tracking-widest">Presenter</p>
          </div>
            ...
            {isUserMenuOpen && (
              ...
              >
                <div className="px-2 py-3 border-b border-white/5 mb-2">
                  ...
                  <p className="text-[10px] font-black text-muted uppercase tracking-widest mt-1">Speaker Identity</p>
                </div>
                <div className="space-y-1">
                  <button 
                    onClick={() => { router.push('/'); setIsUserMenuOpen(false); }}
                  ...
                  <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-all text-[12px] font-bold text-muted hover:text-[#E8EAFF]">
