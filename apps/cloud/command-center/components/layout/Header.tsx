...
import { Check, ChevronDown, HelpCircle, LogOut, Menu, Palette, Settings, User } from "lucide-react";
...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-30 flex min-h-16 w-full items-center gap-2 border-b border-[var(--border-subtle)] bg-[color-mix(in_srgb,var(--base)_88%,transparent)] px-3 backdrop-blur-xl sm:px-4 md:gap-4 md:px-6">
      ...
      <div className="flex shrink-0 items-center justify-end gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-64 border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-primary)]">
            ...
            <DropdownMenuSeparator className="bg-[var(--border-subtle)]" />
            <DropdownMenuItem onSelect={logout} className="text-[var(--status-danger)] focus:text-[var(--status-danger)]"><LogOut aria-hidden className="mr-2 size-4" />Sign out</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </header>
