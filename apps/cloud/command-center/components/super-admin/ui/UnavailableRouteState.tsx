  ...
  return (
    <PageContainer>
      <SectionHeader
        ...
        actions={
          <Badge className="border-[var(--status-warning)]/30 bg-[var(--status-warning-muted)] px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--status-warning)]">
            Contract required
          </Badge>
        }
      />
      ...
      <Card className="overflow-hidden rounded-3xl border border-amber-500/20 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,0.16),transparent_35%),var(--bg-surface)] p-0">
        <div className="grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5 p-7">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[var(--status-warning)]/25 bg-[var(--status-warning-muted)] text-[var(--status-warning)]">
              <ShieldAlert className="h-5 w-5" />
            </div>

            <div className="space-y-2">
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--status-warning)]">Production disabled</p>
                  ...
                  <ul className="space-y-2 text-xs leading-5 text-secondary">
                    {removed.map((item) => (
                      <li key={item} className="flex gap-2">
                        <LockKeyhole className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--status-warning)]" />
                        <span>{item}</span>
                      </li>
                    ))}
                  </ul>
