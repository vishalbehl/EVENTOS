...
export function ThemeSwitcher({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("space-y-4 rounded-[var(--radius-card)] border-default bg-[var(--card)] p-4", className)}>
      ...
      <div className="grid grid-cols-3 gap-2">
        {themes.map((t) => (
          <button
            ...
            className={cn(
              "flex h-10 items-center justify-center gap-2 rounded-lg border px-2 text-[12px] font-semibold transition-all outline-none",
              theme === t.name 
          ...
          >
            <div 
              ...
              style={{ 
                background: t.name === 'dark' ? '#18191c' : t.name === 'light' ? '#FFFFFF' : 'linear-gradient(135deg,#fff 50%,#18191c 50%)'
              }}
            >
              {theme === t.name && (
                <motion.div
