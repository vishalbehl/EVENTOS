...
import { AlertTriangle, CheckCircle2, CircleDollarSign, RefreshCw, RotateCcw, ShieldCheck, Undo2 } from "lucide-react";
...
export default function PaymentsPage() {
  ...
  const receiptQuery = useProviderWebhookReceipts(scope);
  ...
  return <PageContainer>
    ...
    <div className="my-5 flex flex-wrap gap-2">{FILTERS.map((status) => <button key={status} onClick={() => setReconciliationFilter(status)} className={`rounded-lg border px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider ${reconciliationFilter === status ? "border-[var(--brand-primary)] bg-[var(--brand-primary)] text-[var(--primary-foreground)]" : "border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-secondary)]"}`}>{status}</button>)}</div>
    ...
    {supportScope ? <section className="mt-8" aria-labelledby="provider-receipts-heading">
      <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
        <div><h2 id="provider-receipts-heading" className="flex items-center gap-2 text-sm font-semibold text-[var(--text-primary)]"><ShieldCheck className="size-4 text-emerald-400" />Verified provider receipts</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">Signature-verified Stripe and Razorpay callbacks. Raw provider payloads remain restricted to the backend reconciliation ledger.</p></div>
        <Button variant="outline" size="sm" disabled={receiptQuery.isFetching} onClick={() => void receiptQuery.refetch()}><RefreshCw className={`mr-2 size-3.5 ${receiptQuery.isFetching ? "animate-spin" : ""}`} />Refresh receipts</Button>
      </div>
      {receiptQuery.isLoading ? <TableSkeleton rows={4} cols={7} /> : !(receiptQuery.data?.pages.flatMap((page) => page.items).length) ? <EmptyState title="No verified provider receipts" description="Signed provider callbacks linked to this organization will appear here." className="py-10" /> : <div className="overflow-hidden rounded-2xl border border-[var(--border-default)]"><div className="overflow-auto"><table className="w-full text-left text-xs"><thead><tr className="border-b border-[var(--border-default)] bg-[var(--bg-surface-2)]">{["Received", "Provider", "Event", "Provider event", "Invoice", "Transaction", "Outcome"].map((header) => <th key={header} className="h-10 whitespace-nowrap px-4 text-[10px] font-bold uppercase tracking-widest text-[var(--text-tertiary)]">{header}</th>)}</tr></thead><tbody>{receiptQuery.data?.pages.flatMap((page) => page.items).map((receipt) => <tr key={receipt.id} className="border-b border-[var(--border-subtle)]"><td className="whitespace-nowrap px-4 py-3 text-[var(--text-secondary)]">{dateTime(receipt.received_at)}</td><td className="px-4 py-3"><Badge className="border-[var(--border-default)] bg-[var(--bg-surface-2)] text-[var(--text-secondary)]">{receipt.provider}</Badge></td><td className="whitespace-nowrap px-4 py-3 text-[var(--text-secondary)]">{receipt.event_type}</td><td className="max-w-48 truncate px-4 py-3 font-mono text-[10px] text-[var(--text-secondary)]" title={receipt.provider_event_id}>{receipt.provider_event_id}</td><td className="px-4 py-3 font-mono text-[10px] text-[var(--text-secondary)]">{receipt.invoice_id ? `${receipt.invoice_id.slice(0, 8)}...` : "Unlinked"}</td><td className="px-4 py-3 font-mono text-[10px] text-[var(--text-secondary)]">{receipt.transaction_id ? `${receipt.transaction_id.slice(0, 8)}...` : "Pending"}</td><td className="px-4 py-3"><Badge className={`border ${receipt.status === "PROCESSED" ? RECONCILIATION_COLORS.RECONCILED : receipt.status === "REVIEW_REQUIRED" ? RECONCILIATION_COLORS.MISMATCH : RECONCILIATION_COLORS.PENDING}`}>{receipt.status}</Badge>{receipt.failure_code ? <p className="mt-1 max-w-56 text-[10px] text-red-300">{receipt.failure_code}</p> : null}</td></tr>)}</tbody></table></div></div>}
      {receiptQuery.hasNextPage ? <div className="mt-4 flex justify-center"><Button variant="outline" size="sm" disabled={receiptQuery.isFetchingNextPage} onClick={() => void receiptQuery.fetchNextPage()}>{receiptQuery.isFetchingNextPage ? "Loading..." : "Load more receipts"}</Button></div> : null}
    </section> : null}
