...
import { Layout, Users, Grid, Network, ArrowRight, Library, Settings, CheckCircle2 } from "lucide-react"
...
export default function TemplatesLibraryDashboard() {
  ...
  const netCount = data?.network_templates?.length ?? 0

  const totalTemplates = roomCount + regCount + srrCount + netCount
  ...
  const recentTemplates = useMemo(() => {
    ...
    const all = [
      ...
      ...(data.network_templates ?? []).map(t => ({ ...t, type: 'Network infrastructure' }))
    ]
    return all.slice(0, 5) // Return up to 5
  }, [data])
  ...
  return (
    <PageContainer>
      <SectionHeader
        ...
        description="Standardized presets for room planning, attendee registration desks, Speaker Ready Stations, and networking topographies"
      />
      ...
      <MetricRow metrics={[
        ...
        { label: 'Network Topologies', value: netCount },
      ]} />
        ...
        {[
          {
            ...
            count: roomCount,
            href: "/commercial/templates/room",
            icon: Layout,
          ...
          },
          {
            ...
            count: regCount,
            href: "/commercial/templates/registration",
            icon: Users,
          ...
          },
          {
            ...
            count: srrCount,
            href: "/commercial/templates/srr",
            icon: Grid,
            color: "text-indigo-400 bg-indigo-500/10 border-indigo-500/20"
          },
          {
            title: "Network Templates",
            desc: "Venue-wide Wi-Fi layouts, access points, core switches, redundant ISP links and failovers.",
            count: netCount,
            href: "/commercial/templates/network",
            icon: Network,
                ...
                <div key={idx} className="flex justify-between items-center p-3 border border-border/40 rounded-2xl bg-surface-2/30 hover:bg-surface-2/65 transition-colors">
                  <div className="space-y-1">
                    <div className="flex gap-2 items-center">
                      ...
                      <Badge variant="outline" className="text-[9px] px-1 py-0 border-border-subtle bg-surface-2 text-tertiary">
                        {t.type}
                      </Badge>
                    </div>
                    {t.description && (
                      <span className="text-[10px] text-secondary block line-clamp-1">{t.description}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {t.is_default && (
                      <Badge className="bg-success-muted/10 text-success border border-success/20 text-[9px] px-1.5 py-0 font-bold">
                        Default
                      </Badge>
                    )}
        ...
        <div className="bg-[var(--bg-surface-2)] border border-[var(--border-default)] rounded-3xl p-6 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex gap-3 items-start">
                <CheckCircle2 className="h-4 w-4 text-success shrink-0 mt-0.5" />
                <span className="text-secondary font-medium">Integrated Wi-Fi capacity check based on attendee headcounts.</span>
              </div>
            </div>
          </div>
