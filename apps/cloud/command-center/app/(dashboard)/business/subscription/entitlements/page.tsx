...
export default function EntitlementsPage() {
  ...
  return (
    <PageContainer>
      ...
      {subscriptionTarget ? <BillingStatusDialog open onOpenChange={(open) => !open && setSubscriptionTarget(null)} pending={subscriptionMutation.isPending} title="Change subscription status" description="This propagates commercial restrictions to grants and preserves existing activation snapshots for continuity." currentVersion={subscriptionTarget.version} statusOptions={SUBSCRIPTION_TRANSITIONS[subscriptionTarget.status] ?? []} onSubmit={(payload, idempotencyKey) => execute(() => subscriptionMutation.mutateAsync({ subscriptionId: subscriptionTarget.id, payload, idempotencyKey }), "Subscription status updated", () => setSubscriptionTarget(null))} /> : null}
      {grantStatusTarget ? <BillingStatusDialog open onOpenChange={(open) => !open && setGrantStatusTarget(null)} pending={grantStatusMutation.isPending} title="Change grant status" description="New consumption is blocked when restricted; existing event activations follow continuity status policy." currentVersion={grantStatusTarget.version} statusOptions={GRANT_TRANSITIONS[grantStatusTarget.status] ?? []} onSubmit={(payload, idempotencyKey) => execute(() => grantStatusMutation.mutateAsync({ grantId: grantStatusTarget.id, payload, idempotencyKey }), "Grant status updated", () => setGrantStatusTarget(null))} /> : null}
