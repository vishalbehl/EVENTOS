...
const TAB_RESOURCE: Record<TabKey, CRMResourceType> = {
  accounts: "account",
  contacts: "contact",
  leads: "lead",
...
const RESOURCE_LABEL: Record<CRMResourceType, string> = {
  account: "account",
  contact: "contact",
  lead: "lead",
...
function RowActions({
  ...
  record: CRMRecord;
  name: string;
  onEdit: (resourceType: CRMResourceType, record: CRMRecord) => void;
...
function AccountsTab({ records, isLoading, onEdit, onLifecycle }: TabProps<CRMAccount>) {
    ...
    <CRMTable headers={["ID", "Name", "Website", "Industry", "Created", "State", "Actions"]} isEmpty={!records.length} emptyTitle="No accounts found" emptyDescription="No CRM accounts match this tenant scope.">
      {records.map((account) => (
        <tr key={account.id} className={`${ROW_CLS} ${account.archived_at ? "opacity-70" : ""}`}>
          ...
          <td className={CELL_CLS}><span className="font-semibold text-[var(--text-primary)]">{account.name}</span></td>
          ...
          <td className={CELL_CLS}><RowActions resourceType="account" record={account} name={account.name} onEdit={onEdit} onLifecycle={onLifecycle} /></td>
        </tr>
      ))}
    </CRMTable>
  );
...
function LeadsTab({ records, isLoading, onEdit, onLifecycle }: TabProps<CRMLead>) {
  if (isLoading) return <TableSkeleton rows={6} cols={7} />;
  return (
      ...
      {records.map((lead) => {
        ...
        return (
          <tr key={lead.id} className={`${ROW_CLS} ${lead.archived_at ? "opacity-70" : ""}`}>
            ...
            <td className={CELL_CLS}><RowActions resourceType="lead" record={lead} name={name} onEdit={onEdit} onLifecycle={onLifecycle} /></td>
          </tr>
        );
      })}
    </CRMTable>
...
export default function CRMPage() {
  ...
  const [dialog, setDialog] = useState<{ resourceType: CRMResourceType; record: CRMRecord | null } | null>(null);
  ...
  const editRecord = (resourceType: CRMResourceType, record: CRMRecord) => setDialog({ resourceType, record });
    ...
    <PageContainer>
      ...
      <div className="mt-8">
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as TabKey)}>
          ...
          <TabsContent value="accounts"><AccountsTab records={accounts} isLoading={accountsQuery.isLoading} onEdit={editRecord} onLifecycle={setLifecycleTarget} /></TabsContent>
          ...
          <TabsContent value="leads"><LeadsTab records={leads} isLoading={leadsQuery.isLoading} onEdit={editRecord} onLifecycle={setLifecycleTarget} /></TabsContent>
