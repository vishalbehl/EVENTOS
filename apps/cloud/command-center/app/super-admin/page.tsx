  ...
  return (
    <div className="grid grid-cols-2 gap-2 h-full">
      {statuses.map((item) => (
        <div
          key={item.label}
          onClick={() => router.push(`/super-admin/commercial/subscriptions?status=${item.query}`)}
    ...
    <div className="flex items-center justify-between py-2 text-xs border-b border-border/40 last:border-b-0">
      <div className="min-w-0">
        <p className="font-semibold text-[var(--text-primary)] truncate">
          <span className="hover:underline cursor-pointer" onClick={() => router.push(`/super-admin/organizations/${orgId}`)}>
            {orgName}
          </span>
        </p>
