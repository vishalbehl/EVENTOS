...
import { Check, Crown, Layers3, MapPin, Sparkles } from "lucide-react";
  ...
  return (
    <article
      className={cn(
        ...
        plan.isPopular && "ring-1 ring-violet-300/70 dark:ring-violet-400/20"
      )}
      style={{
        borderColor: `${plan.colorHex}55`,
        boxShadow: `0 18px 55px -32px ${plan.colorHex}`,
      }}
    >
      <div className="absolute inset-x-0 top-0 h-24 bg-[linear-gradient(180deg,rgba(79,70,229,0.08),transparent)] dark:bg-[linear-gradient(180deg,rgba(255,255,255,0.06),transparent)]" />
      <div className="relative flex flex-1 flex-col p-6">
        ...
        <div className="mt-auto flex gap-2 pt-7">
          <Button onClick={onAction} className="flex-1 h-11 rounded-xl text-[12px] font-semibold">
            {actionLabel}
          </Button>
          {secondaryLabel ? (
            <Button
              ...
              variant="outline"
              className="flex-1 h-11 rounded-xl border-[var(--border-default)] bg-transparent text-[var(--text-primary)] hover:bg-[var(--bg-surface-2)]"
            >
              {secondaryLabel}
            </Button>
          ) : null}
...
export function CommercialAddonCard({
  ...
  return (
    <article
      className={cn(
        "group flex h-full flex-col overflow-hidden rounded-[28px] border border-[var(--border-default)] bg-[var(--bg-surface)] transition-all duration-200 hover:-translate-y-1",
    ...
    >
      <div className="relative h-52 overflow-hidden bg-black">
        {addon.imageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-[linear-gradient(135deg,#312e81,#7c3aed)]">
        ...
        <div className="absolute inset-x-0 bottom-0 px-4 pb-4 pt-12 text-white">
          <div className="flex items-center justify-between gap-3">
            ...
            <span className={cn("rounded-full px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.2em]", addon.isActive === false ? "bg-white/10 text-white/55" : "bg-white text-black")}>
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mt-3 grid grid-cols-3 border-y border-[var(--border-default)] py-3">
          ...
          <StatBlock label={isVenue ? "Hardware" : "Access"} value={isVenue ? String(addon.hardwareCount || 0) : addon.accessLabel || "Open"} withBorder />
          <StatBlock label={isVenue ? "Staff" : "Type"} value={isVenue ? String(addon.staffCount || 0) : "Plan"} withBorder />
        </div>

        <div className="mt-4 flex items-end justify-between gap-2">
          <div className="min-w-0">
            <div className="text-[9px] uppercase text-[var(--text-tertiary)]">{isVenue ? "Price range" : "Commercial range"}</div>
            <div className="mt-1 truncate text-lg font-semibold text-[var(--text-primary)]">{addon.priceLabel}</div>
          </div>
          <Check className={cn("h-4 w-4 transition-opacity", selected ? "opacity-100 text-violet-500" : "opacity-0 group-hover:opacity-100 text-[var(--text-primary)]")} />
        </div>
        ...
        {actionLabel ? (
          <Button
            ...
            className="mt-auto h-10 rounded-xl border-[var(--border-default)] text-xs font-semibold"
          >
            {actionLabel}
          </Button>
        ) : null}
...
export function TemplateRecommendationCard({
  ...
  return (
    <article className="overflow-hidden rounded-[28px] border border-[var(--border-default)] bg-[var(--bg-surface)] shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
      <div className="relative h-48 overflow-hidden bg-[linear-gradient(135deg,#312e81,#7c3aed)]">
        {template.imageUrl ? (
      ...
      <div className="p-5">
        ...
        <Button onClick={onAction} variant="outline" className="mt-5 h-10 w-full rounded-xl text-xs font-semibold">
          {ctaLabel}
        </Button>
      </div>
    </article>
