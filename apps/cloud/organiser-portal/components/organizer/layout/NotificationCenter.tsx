...
export function NotificationCenter() {
  ...
  useEffect(() => {
    ...
    socket.on("event_notification", handleNotification);
    return () => {
      socket.off("notification", handleNotification);
      socket.off("event_notification", handleNotification);
    };
  }, [socket]);
    ...
    <Sheet>
      <SheetTrigger asChild>
        ...
        >
          ...
          {unread > 0 && (
            <span className="absolute right-1 top-1 min-w-3 rounded-full bg-[var(--status-danger)] px-1 text-[8px] font-bold leading-3 text-white">
              {Math.min(unread, 9)}
            </span>
          )}
        </button>
        ...
        <div className="cc-scroll-region flex-1 overflow-y-auto p-4" aria-live="polite">
          {!items.length ? (
            <div className="flex min-h-80 flex-col items-center justify-center text-center">
              <span className="grid size-12 place-items-center rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface-2)]">
              ...
              <p className="mt-2 max-w-xs text-xs leading-5 text-[var(--text-secondary)]">
                {isConnected
                  ? "New authenticated operational events will appear here."
                  : "The real-time channel is unavailable. Critical status pages remain accessible through normal API requests."}
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              ...
              {items.map((item) => (
                <article key={item.id} className="rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface-2)] p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <time className="mt-2 block font-mono text-[10px] text-[var(--text-tertiary)]" dateTime={item.receivedAt}>
