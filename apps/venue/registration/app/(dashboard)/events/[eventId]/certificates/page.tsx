...
export default function CertificatePrinter() {
  ...
  const handleBulkPrint = async () => {
    ...
    try {
      ...
      const width_mm = template.width_mm || 297;
      ...
      toast.success("Certificates compiled and exported.");
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="flex-1 flex flex-col space-y-6 min-h-0 p-6 max-w-7xl mx-auto">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div>
          <div className="flex items-center gap-2">
            <Award className="h-5 w-5 text-[var(--pri)] animate-pulse" />
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--pri)]/80">Credentials Terminal</span>
          </div>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-0">
          ...
          <Card className="flex flex-col h-full p-8 glass-3d border-default rounded-[2rem] bg-[color-mix(in_srgb,var(--text)_5%,transparent)] group hover-lift-3d relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-6 pr-1 custom-scrollbar min-h-0">
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full h-14 px-6 rounded-2xl border border-default bg-black text-xs font-bold text-[var(--text)] focus:outline-none focus:border-[var(--pri)] transition-all cursor-pointer"
                >
                  {templates.map(t => (
            ...
            <div className="pt-6 shrink-0">
              <Button 
                ...
                className="w-full h-12 bg-[var(--pri)] hover:bg-[var(--sec)] text-white font-black uppercase tracking-widest text-[11px] rounded-full border-0 hover-lift-3d transition-all duration-300 disabled:opacity-40"
              >
        ...
        <div className="lg:col-span-2 flex flex-col min-h-0 h-full space-y-6">
          <Card className="p-5 glass-3d border-default bg-[color-mix(in_srgb,var(--text)_5%,transparent)] grid grid-cols-1 sm:grid-cols-3 gap-4 items-center shadow-lg shrink-0 rounded-[2rem]">
            <div className="relative">
              ...
              <Input 
                ...
                className="h-12 bg-white/5 border-default rounded-full pl-10 pr-6 font-bold text-xs text-[var(--text)] focus:border-[var(--pri)] focus:ring-0 transition-all"
              />
            </div>
            <select 
              value={roleFilter} 
              onChange={e => setRoleFilter(e.target.value)} 
              className="h-12 px-6 rounded-full border border-default bg-black text-[11px] font-black uppercase tracking-widest text-[var(--text)] focus:outline-none focus:border-[var(--pri)] cursor-pointer"
            >
