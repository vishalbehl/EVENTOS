...
export default function ScanningLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[var(--base)] p-6 text-[var(--text)]">
        <section className="max-w-lg rounded-3xl border border-[var(--border)] bg-[var(--card)] p-8 text-center shadow-2xl">
          <p className="text-[10px] font-black uppercase tracking-[0.28em] text-[var(--pri)]">Scanning unavailable</p>
  ...
  const navGroups = [
    {
      label: "GATEWAY SCANNING",
      items: [
        { name: "Live Camera Scanner", href: "/scanning", icon: Camera },
        { name: "Scan History", href: "/scanning/history", icon: History },
      ],
    },
  ];
      ...
      >
        <div className="flex h-full flex-col overflow-visible px-3 py-4">
          ...
          <div className={cn("group/logo relative mb-3 flex h-11 items-center px-2", isCollapsed ? "justify-center" : "justify-between")}>
            <div className={cn("flex min-w-0 items-center", isCollapsed ? "justify-center" : "gap-2.5")} aria-label="Eventos Scanner">
              <span
                className={cn(
              ...
              {!isCollapsed && (
                <div className="whitespace-nowrap">
                  ...
                  <span className="block truncate text-[9px] font-bold tracking-widest text-emerald-500 mt-1">
                    Scanner Gateway
                  </span>
                </div>
              )}
          ...
          {!isCollapsed && (
            <div className="relative mb-4">
              ...
              >
                <span className="grid size-8 shrink-0 place-items-center rounded-lg border border-[var(--border)] bg-[var(--surf)] text-emerald-500">
                  <Box className="size-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-xs font-bold text-[var(--text)]">Scanning Terminal</span>
          ...
          {/* Navigation Items */}
          <nav
            aria-label="Scanning Navigation"
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          active
                            ? "bg-emerald-600 text-white shadow-[inset_0_0_0_1px_var(--border)]"
          ...
          <div className="shrink-0 border-t border-[var(--border)] pt-2">
            <div className="space-y-1 pb-2">
              {[
                { name: "Documentation", href: "/scanning/documentation", icon: HelpCircle },
                { name: "Settings", href: "/scanning/settings", icon: Settings },
              ].map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href} className={cn(
                    ...
                    active ? "bg-emerald-600 text-white" : "text-[var(--muted)] hover:bg-[var(--raised)] hover:text-[var(--text)]",
                    isCollapsed && "justify-center px-0"
                  )}>
                    <Icon className={cn("size-4 shrink-0", !isCollapsed && "mr-3")} />
            ...
            <div className="border-t border-[var(--border)] pt-2 text-center text-[10px] text-[var(--muted)] font-mono">
              {!isCollapsed && <span>EventOS v1.0 · Gate Mode</span>}
            </div>
          </div>
        </div>
      </motion.aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-16 bg-[var(--surf)] border-b border-[var(--border)] flex items-center justify-between px-6 shrink-0 gap-4 z-20">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 text-xs font-bold uppercase tracking-wider">
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              >
                <div className="h-8 w-8 rounded-lg bg-emerald-600 text-white flex items-center justify-center font-black text-xs">
                  {((user as any)?.full_name || (user?.first_name ? `${user.first_name} ${user.last_name || ""}`.trim() : "Gatekeeper")).slice(0, 2).toUpperCase()}
                </div>
                <div className="hidden lg:block text-left pr-1">
                  <div className="text-xs font-black text-[var(--text)] leading-none">
                    {(user as any)?.full_name || (user?.first_name ? `${user.first_name} ${user.last_name || ""}`.trim() : "Gatekeeper")}
                  </div>
                  <div className="text-[10px] text-[var(--muted)] font-semibold uppercase mt-0.5">{user?.role || "Gate Operator"}</div>
                </div>
              </button>
              ...
              {isUserMenuOpen && (
                <div className="venue-profile-menu absolute right-0 mt-2 w-56 rounded-2xl p-2 z-50 space-y-1">
                  <div className="px-3 py-2 border-b border-[var(--border)]">
                    ...
                    <p className="text-[10px] text-[var(--muted)] font-medium truncate">{user?.email || "gatekeeper@eventos.com"}</p>
                  </div>

                  <Link href="/scanning/settings" onClick={() => setIsUserMenuOpen(false)} className="venue-profile-menu-item text-xs font-bold">
                      <User className="w-4 h-4 text-emerald-500" />
                      <span>My Profile & Settings</span>
                  </Link>

                  <Link href="/scanning/documentation" onClick={() => setIsUserMenuOpen(false)} className="venue-profile-menu-item text-xs font-bold">
