...
export default function ConnectivityPage() {
  ...
  const venue = data?.venue_server;
  const sync = data?.sync;
  const topologies = (data?.topology_modes || {}) as Record<string, string>;
    ...
    <div className="mx-auto flex w-full max-w-[1720px] flex-col gap-6 pb-12">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-blue-500/30 bg-blue-950/15 p-5 shadow-sm space-y-2">
          ...
          <div className="text-2xl font-black text-blue-300">● {cloud?.status || "UNKNOWN"}</div>
          <p className="text-xs text-[var(--muted)]">Global master event configuration & assets</p>
          <div className="font-mono text-[11px] text-blue-400 font-bold">Latency: {cloud?.latency_ms ?? "Unknown"} ms (Roundtrip)</div>
        </div>
        ...
        <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-5 shadow-sm space-y-2">
          ...
          <div className="text-2xl font-black text-emerald-300">● {venue?.status || "UNKNOWN"}</div>
          <p className="text-xs text-[var(--muted)]">Local authoritative edge broker & distribution controller</p>
          <div className="font-mono text-[11px] text-emerald-400 font-bold">Node ID: {venue?.node_id || "Not reported"}</div>
        </div>
        ...
        <div className="rounded-2xl border border-purple-500/30 bg-purple-950/15 p-5 shadow-sm space-y-2">
          ...
          <div className="text-2xl font-black text-purple-300">● {data?.local_services_summary || "UNKNOWN"}</div>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surf)] p-3">
              ...
              <div className="mt-1 text-sm font-black text-emerald-400">✓ {sync?.cloud_to_venue || "Unknown"}</div>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surf)] p-3">
              <span className="font-mono text-[9px] uppercase text-[var(--muted)]">Venue → Cloud Outbox</span>
              <div className="mt-1 text-sm font-black text-[var(--text)]">{sync?.venue_to_cloud_pending ?? "Unknown"} Pending</div>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surf)] p-3">
              <span className="font-mono text-[9px] uppercase text-[var(--muted)]">Sync Conflicts</span>
              <div className="mt-1 text-sm font-black text-emerald-400">{sync?.conflicts ?? "Unknown"} Conflicts</div>
            </div>
            <div className="rounded-xl border border-[var(--border)] bg-[var(--surf)] p-3">
              <span className="font-mono text-[9px] uppercase text-[var(--muted)]">Snapshot Version</span>
              <div className="mt-1 font-mono text-sm font-black text-[var(--acc)]">#{sync?.event_version ?? "Unknown"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-2 text-xs">
            {Object.keys(topologies).length === 0 ? (
              <div className="rounded-xl border border-[var(--border)] bg-[var(--surf)] p-4 text-center text-xs text-[var(--muted)]">
                {isLoading ? "Loading topology modes..." : "No topology metadata reported yet"}
              </div>
            ) : (
